/*
 * USDZExporter from three.js r170 (MIT, https://github.com/mrdoob/three.js), adapted for
 * ar-kit/ar-export.js. Changes from the original:
 *  - textures flagged userData.mimeType = 'image/jpeg' are stored as .jpg (Quick Look reads JPEG;
 *    PNG-only made a lit endcap ~4x bigger)
 *  - meshes whose material is DoubleSide get `doubleSided = 1` instead of a warning
 *  - emissive colour without a map includes emissiveIntensity (clamped to 1)
 *  - coordinates written with fewer digits (sub-millimetre at metre scale) to keep the file small
 */
import {
	NoColorSpace,
	DoubleSide,
...
import {
	strToU8,
	zipSync,
} from '../three@0.170.0/jsm/libs/fflate.module.js';
...
class USDZExporter {
	...
	async parseAsync( scene, options = {} ) {
		...
		scene.traverseVisible( ( object ) => {
			...
			if ( object.isMesh ) {
				...
				if ( material.isMeshStandardMaterial ) {
					...
					const geometryKey = geometry.id + ( doubleSided ? '_ds' : '' );
					const geometryFileName = 'geometries/Geometry_' + geometryKey + '.usda';

					if ( ! ( geometryFileName in files ) ) {

						const meshObject = buildMeshObject( geometry, doubleSided );
					...
					output += buildXform( object, geometryKey, material );
		...
		for ( const id in textures ) {
			...
			const jpeg = texture.userData.mimeType === 'image/jpeg';
			const blob = await new Promise( resolve => canvas.toBlob( resolve, jpeg ? 'image/jpeg' : 'image/png', jpeg ? 0.9 : 1 ) );

			files[ `textures/Texture_${ id }.${ jpeg ? 'jpg' : 'png' }` ] = new Uint8Array( await blob.arrayBuffer() );
...
function buildXform( object, geometryKey, material ) {
	...
	prepend references = @./geometries/Geometry_${ geometryKey }.usda@</Geometry>
...
function buildMeshObject( geometry, doubleSided ) {

	const mesh = buildMesh( geometry, doubleSided );
	return `
def "Geometry"
{
${mesh}
...
function buildMesh( geometry, doubleSided ) {
	...
	def Mesh "${ name }"
	{
		...
		normal3f[] normals = [${ buildVector3Array( attributes.normal, count, 4 )}] (
			interpolation = "vertex"
		)
		point3f[] points = [${ buildVector3Array( attributes.position, count )}]
${ buildPrimvars( attributes ) }
		uniform token subdivisionScheme = "none"${ doubleSided ? `
...
function buildVector3Array( attribute, count, precision = 6 ) {
...
function buildVector2Array( attribute ) {
	...
	for ( let i = 0; i < attribute.count; i ++ ) {
		...
		array.push( `(${ +x.toPrecision( 5 ) }, ${ +( 1 - y ).toPrecision( 5 ) })` );
...
function buildMaterial( material, textures, quickLookCompatible = false ) {
	...
	function buildTexture( texture, mapType, color ) {
		...
		const id = texture.source.id + '_' + texture.flipY + ( texture.userData.mimeType === 'image/jpeg' ? '_j' : '' );
		...
		def Shader "Texture_${ texture.id }_${ mapType }"
		{
			...
			asset inputs:file = @textures/Texture_${ id }.${ texture.userData.mimeType === 'image/jpeg' ? 'jpg' : 'png' }@
	...
	if ( material.emissiveMap !== null ) {
		...
		inputs.push( `${ pad }color3f inputs:emissiveColor = ${ buildColor( new Color( Math.min( 1, e.r ), Math.min( 1, e.g ), Math.min( 1, e.b ) ) ) }` );
