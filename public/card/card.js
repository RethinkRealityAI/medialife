...
(() => {
  ...
  const QR = {
    contact: {
      src: base + "qr-contact.svg",
      alt: "QR code — scan to save contact",
      caption: "SCAN TO SAVE CONTACT",
      sub: "Scan to save contact",
      hint: 'Point any phone camera at the code — tap "Add contact"',
    },
    link: {
      src: base + "qr-link.svg",
      alt: "QR code — scan to open this card",
      caption: "SCAN TO OPEN CARD",
      sub: cardUrl,
      hint: "Point any phone camera at the code",
    },
  };
  const setQr = (mode) => {
  ...
  let targetX = 0,
    ...
    raf;
  const MAX = 7;
  const setSheen = (px, py) => {
    root.setProperty("--sheen-x", `${px * 100}%`);
    root.setProperty("--sheen-y", `${py * 100}%`);
  };
  const tick = () => {
    curX += (targetX - curX) * 0.12;
    curY += (targetY - curY) * 0.12;
    root.setProperty("--tilt-x", `${curX.toFixed(2)}deg`);
    root.setProperty("--tilt-y", `${curY.toFixed(2)}deg`);
  ...
  const colors = [
    css.getPropertyValue("--primary").trim() || "#19affe",
    css.getPropertyValue("--accent").trim() || "#ff37ae",
    "#ffffff",
  ];
  let W,
    H,
