...
(function () {
  ...
  function buildRail() {
    STAGES.forEach(function (id, idx) {
      ...
      b.innerHTML =
        '<span class="sr-only">Go to stage ' + (idx + 1) + ": " + LABELS[id] + "</span>";
  ...
  function syncRail() {
    $$("button", railEl).forEach(function (b, idx) {
      if (idx === S.i) b.setAttribute("aria-current", "step");
  ...
  function next() {
    go(STAGES[Math.min(S.i + 1, STAGES.length - 1)]);
  }
  ...
  function schedule(id) {
    ...
    if (id === "activating") {
      later(next, reduced() ? 700 : DUR.activating);
      return;
    }
  ...
  function emit(id) {
    ...
    try {
      doc.dispatchEvent(new CustomEvent("medialife:stage", { detail: payload }));
    } catch (e) {
      /* old browser */
    }
  }
  ...
  var ACT_STEPS = [
    [0, "NFC HANDSHAKE"],
    [26, "RESOLVING TAG 8F-2C"],
    [55, "STREAMING EXPERIENCE"],
    [82, "RENDERING SCENE"],
  ];
  ...
  function runActivating() {
    ...
    function tick() {
      ...
      for (var i = 0; i < ACT_STEPS.length; i++) {
        if (v >= ACT_STEPS[i][0]) label = ACT_STEPS[i][1];
      }
  ...
  function openLore(i) {
    ...
    $$(".hotspot").forEach(function (h, idx) {
      h.setAttribute("aria-expanded", idx === i ? "true" : "false");
    });
    ...
    if (!S.hot[i]) {
      S.hot[i] = true;
      S.hotCount++;
      log("hotspot.open", d.t.toUpperCase());
    }
    clearTimers(); // reading time is not dead time — hold the sequence
    try {
      $("#loreClose").focus({ preventScroll: true });
    } catch (e) {
      $("#loreClose").focus();
    }
  }
  ...
  function reveal(auto) {
    ...
    if (!auto) {
      /* manual flip keeps the running timer */
    }
  }
  ...
  function hasWebGL() {
    try {
      ...
      return !!(
        ...
        (c.getContext("webgl2") || c.getContext("webgl", { failIfMajorPerformanceCaveat: false }))
      );
    } catch (e) {
      return false;
    }
  ...
  function boot3D() {
    return Promise.all([
      ...
      .then(function (m) {
        var THREE = m[0];
        var EffectComposer = m[1].EffectComposer,
          RenderPass = m[2].RenderPass;
        var UnrealBloomPass = m[3].UnrealBloomPass,
          OutputPass = m[4].OutputPass;
        ...
        var CYAN = new THREE.Color(0x19affe),
          MAGENTA = new THREE.Color(0xff37ae);
        ...
        var ringA = new THREE.Mesh(
          new THREE.TorusGeometry(2.42, 0.007, 3, 180),
          new THREE.MeshBasicMaterial({ color: 0x19affe, transparent: true, opacity: 0.85 }),
        );
        ringA.rotation.set(1.18, 0.35, 0);
        var ringB = new THREE.Mesh(
          new THREE.TorusGeometry(2.78, 0.006, 3, 180),
          new THREE.MeshBasicMaterial({ color: 0xff37ae, transparent: true, opacity: 0.7 }),
        );
  ...
  function init() {
    ...
    $$("[data-go]").forEach(function (b) {
      b.addEventListener("click", function () {
        go(b.getAttribute("data-go"), true);
      });
    });
    ...
    $$(".hotspot").forEach(function (h) {
      h.addEventListener("click", function () {
        ...
        if (loreOpenIndex === i) closeLore();
        else openLore(i);
      });
    });
    $("#loreClose").addEventListener("click", function () {
      closeLore();
    });
    ...
    $("#claimBtn").addEventListener("click", function (e) {
      ...
      if (!S.cta) {
        S.cta = true;
        log("cta.tap", "DEEP LINK → ROBLOX EXPERIENCE");
      }
      clearTimers();
    ...
    doc.addEventListener("keydown", function (e) {
      if (e.key === "Escape" && loreOpenIndex >= 0) {
        e.preventDefault();
        closeLore();
      }
    });
